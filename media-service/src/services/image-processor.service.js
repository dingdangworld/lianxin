...
class ImageProcessorService {
  ...
  /**
   * Generate all required variants for Facebook-like processing
   */
      ...
      for (const size of processingConfig.sizes) {
        try {
          ...
          const dimensions = this.calculateOptimalDimensions(
            ...
            size.width,
            size.height,
            mediaType === "profile" // Square crop for profile images
          );

          // Generate JPEG variant
          const jpegVariant = await this.generateVariant(
            inputPath,
            outputDir,
            baseFilename,
            size.name,
          ...
          // Generate WebP variant for better compression
          const webpVariant = await this.generateVariant(
            inputPath,
            outputDir,
            baseFilename,
            size.name,
          ...
          logger.warn("Failed to generate variant", {
            size: size.name,
  ...
  async processLivePhoto(imagePath, videoPath, outputDir) {
    try {
      ...
      logger.info("Live photo processing completed", {
        imagePath,
        videoPath,
        imageVariants: imageResult.variants.length,
  ...
  async applyWatermark(inputPath, outputPath, watermarkPath, options = {}) {
    try {
      const { position = "bottom-right", opacity = 0.7, margin = 20 } = options;
      ...
      const watermark = await sharp(watermarkPath)
        .resize(Math.floor(metadata.width * 0.2)) // 20% of image width
        .png()
        .toBuffer();
      ...
      const positions = {
        ...
        "top-right": { top: margin, left: metadata.width - margin },
        "bottom-left": { top: metadata.height - margin, left: margin },
        "bottom-right": {
          top: metadata.height - margin,
          left: metadata.width - margin,
        },
        center: {
