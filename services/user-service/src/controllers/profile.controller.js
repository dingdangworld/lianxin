 ...
 */
router.get("/profile", authMiddleware.authenticate, async (req, res, next) => {
  try {
...
router.put(
  ...
  [
    ...
    body("relationship_status")
      .optional()
      .isIn(["single", "in_relationship", "married", "complicated"])
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const coverPhotoUrl = await profileService.uploadCoverPhoto(
        userId,
        req.file
      );
